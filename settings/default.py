...
def get_joe_tickers():
    #return [ x.replace('.csv', '') for x in os.listdir(os.path.join('data', 'joe')) ][:5]
    return [ 'AAPL' ]

def pull_joe_data(ticker):
    return pd.read_csv(os.path.join('data', 'joe', ticker + '.csv'), parse_dates=True, index_col=0)
