...
class TftDeepMomentumNetworkModel(DeepMomentumNetworkModel):
    def __init__(self, project_name, hp_directory, hp_minibatch_size=HP_MINIBATCH_SIZE, **params):
        params = params.copy()
        print(f"Eto project name ng TFTDeepMomentumNetworkModel: {project_name}")
        print(f"Eto hp_directory name ng TFTDeepMomentumNetworkModel: {hp_directory}")
        print(f"Eto hp_minibatch_size name ng TFTDeepMomentumNetworkModel: {hp_minibatch_size}")
        print(f"Eto params name ng TFTDeepMomentumNetworkModel: {params}")
        ...
        print("DUMAAN SA INITIALIZATION NG TftDeepMomentumNetworkModel")
        super().__init__(project_name, hp_directory, hp_minibatch_size, **params)

    def model_builder(self, hp):
        print("NASA MODEL BUILDER NAKO")
        print(f'Eto hp {hp}')
        ...
        print("LAMPAS NG TFT EMBEDDINGS")
        ...
        #         axis=-1,
        #     )
        if unknown_inputs.shape[-1] != 0:
            historical_inputs = keras.layers.Concatenate(axis=-1)(
            [unknown_inputs, known_combined_layer]
            )
        else:
            historical_inputs = known_combined_layer
        print("LAMPAS NG UNKNOWN INPUTS")
        def static_combine_and_mask(embedding):
            ...
            if len(embedding.shape) == 2:
                # If embedding has only two dimensions, reshape it to add an extra dimension
                embedding = keras.layers.Reshape((1, embedding.shape[1]))(embedding)

            if len(embedding.shape) != 3:
                raise ValueError("Expected embedding to have 3 dimensions, got {}".format(len(embedding.shape)))

            num_static = embedding.shape[1]
            static_dim = embedding.shape[2]
            flatten = keras.layers.Reshape([num_static * static_dim])(embedding)
            # flatten = tf.reshape(
            #     embedding, tf.concat([shape[:-2], [num_static * static_dim]], axis=-1)
            # )
            ...
            sparse_weights = Lambda(lambda x: K.expand_dims(x, axis=-1))(sparse_weights)
            ...
            static_vec = Lambda(lambda x: K.sum(x, axis=1))(combined)
        ...
        print("LAMPAS NG STATIC ENCODER, STATIC WEIGHTS")
        ...
        print("LAMPAS NG GATED RESIDUAL NETWORK")
        def lstm_combine_and_mask(embedding, static_inputs=None, static_context_variable_selection=None):
            ...
            # Add temporal features
            static_shape = K.int_shape(embedding)
            print("Static shape of embedding:", static_shape)
            if len(static_shape) < 4:
                raise ValueError("The input tensor must have at least 4 dimensions.")
            time_steps = static_shape[-3]
            embedding_dim = static_shape[-2]
            num_inputs = static_shape[-1]
            if time_steps is None or embedding_dim is None or num_inputs is None:
                raise ValueError("Time steps, embedding dimension, and number of inputs must be statically defined.")
            ...
            flatten_shape = (-1, time_steps, embedding_dim * num_inputs)
            flatten = keras.Layers.Reshape(flatten_shape)(embedding)
            print("Shape after flatten:", flatten.shape)
            if static_inputs is not None and static_context_variable_selection is not None:
                ...
                expanded_static_context = expand_layer(static_context_variable_selection)
            else:
            ...
            sparse_weights = keras.layers.Activation("softmax")(mlp_outputs)
            sparse_weights = Lambda(lambda x: K.expand_dims(x, axis=-2))(sparse_weights)
            ...
            transformed_embedding = Lambda(lambda x: K.stack(x, axis=-1))(trans_emb_list)

            combined = keras.layers.multiply([sparse_weights, transformed_embedding])
            temporal_ctx = Lambda(lambda x: K.sum(x, axis=-1))(combined)  

            return temporal_ctx, sparse_weights
        #input_embeddings, flags, _ = lstm_combine_and_mask(historical_inputs)
        input_embeddings, flags = lstm_combine_and_mask(historical_inputs)
        print("LAMPAS NG input_embeddings, flags")
        # LSTM layer
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        """
        print("NAGSIMULA NG TFT EMBEDDINGS")
        time_steps = self.time_steps
        ...
        print(f'ETO ANG self.CATEGORY COUNTS: {self.category_counts}')
        embedding_sizes = [
        ...
        if self._static_input_loc:
            ...
            static_inputs = keras.layers.Concatenate(axis=1)(static_inputs)
        ...
        # else:
        #     unknown_inputs = None
        if unknown_inputs or wired_embeddings:
            unknown_inputs = keras.layers.Concatenate(axis=-1)(unknown_inputs + wired_embeddings)
        else:
            # Create a dummy tensor to ensure it's a valid Keras tensor
            unknown_inputs = keras.layers.Input(shape=(time_steps, 0))
        ...
        # )
        known_combined_layer = keras.layers.Concatenate(axis=-1)(known_regular_inputs + known_categorical_inputs)
        print(f"unknown_inputs type: {type(unknown_inputs)}, shape: {unknown_inputs.shape if unknown_inputs is not None else 'None'}")
        print(f"known_combined_layer type: {type(known_combined_layer)}, shape: {known_combined_layer.shape}")
        print(f"static_inputs type: {type(static_inputs)}, shape: {static_inputs.shape if static_inputs is not None else 'None'}")
